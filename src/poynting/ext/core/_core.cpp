...
static FourVector vector_potential_derivative(
    ...
    // Create displaced points in the nu direction
    FourVector point_p = field_point + FourVector::unit(nu) * 0.5;
    FourVector point_m = field_point - FourVector::unit(nu) * 0.5;

    // Compute potential at the displaced points
 ...
 */
static TwoForm faraday_tensor(const ParticleTrajectory& trajectory, const FourVector& field_point, double epsilon) {
    OneForm dA[4] = {
        lower(vector_potential_derivative(trajectory, field_point, 0, epsilon)),
        lower(vector_potential_derivative(trajectory, field_point, 1, epsilon)),
        lower(vector_potential_derivative(trajectory, field_point, 2, epsilon)),
        lower(vector_potential_derivative(trajectory, field_point, 3, epsilon))
    };
    TwoForm F;
